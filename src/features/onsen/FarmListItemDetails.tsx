...
import { ChainId, CurrencyAmount, JSBI, MASTERCHEF_ADDRESS, MASTERCHEF_V2_ADDRESS, Token, ZERO } from '@fogofwar/sdk'
...
const FarmListItem = ({ farm }) => {
  ...
  const token0Amount = CurrencyAmount.fromRawAmount(farm.pair.token0, JSBI.BigInt(token0Reserve))
    .multiply(Math.round(poolFraction * 1e8))
    .divide(1e8)
  const token1Reserve = farm.pool.reserves ? (farm.pool.reserves.reserve1 as BigNumber).toString() : 0
  const token1Amount = CurrencyAmount.fromRawAmount(farm.pair.token1, JSBI.BigInt(token1Reserve))
    .multiply(Math.round(poolFraction * 1e8))
    .divide(1e8)
          ...
          <div className="col-span-2 text-center md:col-span-1">
            {account && (
              <div>
                ...
                <div className="pr-4 mb-2 text-sm text-right cursor-pointer text-secondary">
                  {token0Amount.toFixed(token0Amount.currency.decimals > 2 ? 2 : undefined)} {token0Name} +{' '}
                  {token1Amount.toFixed(token1Amount.currency.decimals > 2 ? 2 : undefined)} {token1Name} (
